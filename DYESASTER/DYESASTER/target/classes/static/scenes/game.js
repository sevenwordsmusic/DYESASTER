var GameScene = new Phaser.Class({
    ...
    {	 
		...
		this.anims.create({
			key: 'walk3',
			frames: this.anims.generateFrameNames('player3', {prefix: 'p1_walk', start: 1, end: 11, zeroPad: 2}),
			frameRate: 15,
			repeat: -1
		});
		...
		this.anims.create({
			key: 'idle3',
			frames: [{key: 'player3', frame: 'p1_stand'}],
			frameRate: 15,
		});
		...
		this.anims.create({
			key: 'walk4',
			frames: this.anims.generateFrameNames('player4', {prefix: 'p1_walk', start: 1, end: 11, zeroPad: 2}),
			frameRate: 15,
			repeat: -1
		});
		// idle with only one frame, so repeat is not neaded
		this.anims.create({
			key: 'idle4',
			frames: [{key: 'player4', frame: 'p1_stand'}],
			frameRate: 15,
		});
    ...
    update: function (time, delta) {
			...
			for(var i=0; i<game.global.length; i++) {
				...
				if(player[i].body){			
					switch(game.global.player[i].direction){
						case 'left':
							player[i].anims.play(walkTextureUpdate(i), true); // walk left
							player[i].flipX = true; // flip the sprite to the left
						break
						case 'right':
							player[i].anims.play(walkTextureUpdate(i), true);
							player[i].flipX = false; // use the original sprite looking to the right
						break
						default :
							player[i].anims.play(idleTextureUpdate(i), true);
						break
					}
		    	}
			}
		...
		//Checks for color (SPACE) to be pressed in order to change the color
		colorCL+=1;
		if(cursors.color.isDown && colorCL>10 ){
				colorCL = 0;
				msg.event = 'UPDATE_COLOR';
				game.global.socket.send(JSON.stringify(msg));
		}
...
// Returns walk animation to use with each player colorId
function walkTextureUpdate(playerId){
	var walkAnim;
	switch(game.global.player[playerId].colorId){
	case 0:
		walkAnim = "walk";
		break;
	case 1:
...
//Returns idle animation to use with each player colorId
function idleTextureUpdate(playerId){
	var walkAnim;
	switch(game.global.player[playerId].colorId){
	case 0:
		walkAnim = "idle";
...
/*function createPlayerSprite(auxi,texture){
	player[auxi] = this.physics.add.sprite(200, 8688, texture);
	player[auxi].setBounce(0.2); // our player will bounce from items
	player[auxi].setCollideWorldBounds(true); // don't go out of the map
	// small fix to our player images, we resize the physics body object slightly
	player[auxi].body.setSize(player[0].width, player[0].height-8);
}*/
